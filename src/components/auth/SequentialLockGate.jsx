...
export default function SequentialLockGate() {
  ...
  const [activeMethod, setActiveMethod] = useState(null); // 'pin' | 'biometric'
  ...
  // Decide initial method when lock activates
  useEffect(() => {
    if (!isLocked) {
    ...
    // Determine which method(s) are available
    if (isBiometricEnabled && isPinEnabled) {
      // Both enabled → start with biometric (user can fall back to PIN)
      ...
      // No lock enabled (shouldn't happen, but unlock safely)
      setLocked(false);
    }
  ...
  useEffect(() => {
    if (pin.length === 4 && activeMethod === 'pin' && !verifying && !lockedOut) {
      handleVerifyPin();
    }
  }, [pin, activeMethod, verifying, lockedOut]);
            ...
            <>
              <TouchableOpacity
                style={[
                  ...
                  !!errorMsg && styles.fingerprintButtonError,
                ]}
              ...
              <Text style={styles.tapText}>
                {errorMsg || 'Tap to scan your biometric'}
              </Text>
            </>
          )}
          {/* Show PIN fallback only if PIN is also enabled */}
          {isPinEnabled && !biometricActive && (
            <TouchableOpacity
              onPress={() => {
                setActiveMethod('pin');
                setPin('');
